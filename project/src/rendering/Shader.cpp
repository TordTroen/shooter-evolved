...
void Shader::load()
{
    ...
    if (std::filesystem::exists(m_vertPath))
    {
        m_vertMtime = std::filesystem::last_write_time(m_vertPath);
    }
    if (std::filesystem::exists(m_fragPath))
    {
        m_fragMtime = std::filesystem::last_write_time(m_fragPath);
    }
}
...
{
    bool changed = false;
    if (std::filesystem::exists(m_vertPath) && std::filesystem::last_write_time(m_vertPath) != m_vertMtime)
    {
        changed = true;
    }
    if (std::filesystem::exists(m_fragPath) && std::filesystem::last_write_time(m_fragPath) != m_fragMtime)
    {
        changed = true;
    }
    if (changed)
